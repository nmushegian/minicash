...
// We save the prototype in-memory js lib version here for reference
...
class MemTree {
    ...
    look(copy :Snap, look :((Rock,Twig) => void)) {
        ...
        if (!snap) throw new Error(`read_twig: no such snap: ${copy}`)
        else {
            let ret
            this.rock.rite(rite => {
                let twig = new MemTwig(snap, rite) // todo, readonly
                look(this.rock, twig)
            })
        }
    }
    ...
    grow(copy :Snap, grow :((Rock,Twig,Snap) => void)) {
        ...
        let next = this._aloc()
        this.rock.rite(rite => {
            let twig = new MemTwig(prev, rite)
            grow(rite, twig, next)
            let immu = twig.seal()
